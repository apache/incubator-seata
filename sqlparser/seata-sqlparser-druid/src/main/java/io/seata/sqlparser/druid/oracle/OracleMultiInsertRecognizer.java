...
public class OracleMultiInsertRecognizer extends BaseOracleRecognizer implements SQLInsertRecognizer {
    ...
    public String getTableName() {
        ...
        visitor.visit(((OracleMultiInsertStatement.InsertIntoClause)ast.getEntries().get(0)).getTableSource());
        return sb.toString();
    }

    @Override
    public boolean insertColumnsIsEmpty() {
        return CollectionUtils.isEmpty(((OracleMultiInsertStatement.InsertIntoClause)ast.getEntries().get(0)).getColumns());
    }

    @Override
    public List<String> getInsertColumns() {
        List<SQLExpr> columnSQLExprs = ((OracleMultiInsertStatement.InsertIntoClause)ast.getEntries().get(0)).getColumns();
    ...
    public List<List<Object>> getInsertRows(Collection<Integer> primaryKeyIndex) {
        int count =  ast.getEntries().size();
        List<List<Object>> rows = new ArrayList<>(ast.getEntries().size());
        for(int j = 0;j < count; j++) {
            List<SQLInsertStatement.ValuesClause> valuesClauses = ((OracleMultiInsertStatement.InsertIntoClause) ast.getEntries().get(j)).getValuesList();
