...
public class BranchSession implements Lockable, Comparable<BranchSession>, SessionStorable {
     ...
     */
    public BranchSession setRollbackType(RollbackType rollbackType) {
    ...
    public boolean canBeRollbackedAsync() {
        return rollbackType == RollbackType.AsyncRollback || rollbackType == RollbackType.NoRollback;
    }
    ...
    public byte[] encode() {
        ...
        byte rollbackTypeByte = rollbackType != null ? (byte) rollbackType.value() : -1;
        ...
        byteBuffer.put(rollbackTypeByte);
    ...
    public void decode(byte[] a) {
        ...
        try {
            int rollbackTypeCode = byteBuffer.get();
            if (rollbackTypeCode >= 0) {
                this.rollbackType = RollbackType.get(rollbackTypeCode);
            }
        } catch (BufferUnderflowException e) {
            if (this.branchType != null) {
                // If the `byte[]` was generated by an older version, set default rollbackType
                this.rollbackType = RollbackType.getDefault(this.branchType);
            }
        }
    }
