...
public class ConfigCenterController {
    ...
    private Logger LOGGER = LoggerFactory.getLogger(ConfigCenterController.class);
    ...
    SingleResult<Map<String, String>> getConfigCenterConf(String key) {
        Configuration instance = ConfigurationFactory.getInstance();
        Map<String, String> configurations = new HashMap<>();
        if (key == null) {
            Set<String> availableProperties = ConfigValidator.availableConfigCenterConf();
            for (String item: availableProperties) {
                configurations.put(item, instance.getConfig(item));
            }
        } else {
            if (ConfigValidator.availableConfigCenterConf().contains(key)) {
    ...
    SingleResult<Map<String, String>> setConfigCenterConf(@RequestBody Map<String, String> properties) {
        Map<String, String> result = new HashMap<>(properties.size());
        for (String key: properties.keySet()) {
            ConfigValidator.ValidateResult validateResult = ConfigValidator.validateCenterConf(key, properties.get(key));
