...
/**
 * Default StateLogStore by database
 *
 * @author wt-better
 */
public class DbStateLogStore extends AbstractStore implements StateLogStore {
     ...
     *
     * @param stateInstance stateInstance
    ...
    private String generateRetryStateInstanceId(StateInstance stateInstance) {
        ...
        while (StringUtils.hasLength(originalStateInst.getStateIdRetriedFor())) {
            ...
            maxIndex = Math.max(idIndex, maxIndex);
            maxIndex++;
        }
    ...
    protected boolean isUpdateMode(StateInstance stateInstance, ProcessContext context) {
        StateMachineConfig stateMachineConfig = (StateMachineConfig) context.getVariable(DomainConstants.VAR_NAME_STATEMACHINE_CONFIG);
        if (stateMachineConfig instanceof AbstractStateMachineConfig) {
            AbstractStateMachineConfig config = (AbstractStateMachineConfig) stateMachineConfig;
            ...
            if (StringUtils.hasLength(stateInstance.getStateIdRetriedFor())) {
                ...
                return config.isSagaRetryPersistModeUpdate();
                ...
                for (int i = 0; i < stateInstance.getStateMachineInstance().getStateList().size(); i++) {
                    ...
                    if (aStateInstance.isForCompensation() && aStateInstance.getName().equals(stateInstance.getName())) {
                        ...
                        return config.isSagaCompensatePersistModeUpdate();
                    }
                }
                return false;
            }
    ...
    public void recordStateFinished(StateInstance stateInstance, ProcessContext context) {
        if (stateInstance != null) {
            ...
            StateMachineConfig stateMachineConfig = (StateMachineConfig) context.getVariable(DomainConstants.VAR_NAME_STATEMACHINE_CONFIG);
            if (!(stateMachineConfig instanceof AbstractStateMachineConfig && !((AbstractStateMachineConfig) stateMachineConfig).isRmReportSuccessEnable() && ExecutionStatus.SU.equals(stateInstance.getStatus()))) {
    ...
    public List<StateInstance> queryStateInstanceListByMachineInstanceId(String stateMachineInstanceId) {
        ...
        if (lastStateInstance != null && lastStateInstance.getGmtEnd() == null) {
