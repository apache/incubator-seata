...
public class DbAndReportTcStateLogStore extends DbStateLogStore {
    ...
    public void branchRegister(StateInstance stateInstance, ProcessContext context) {
        if (sagaTransactionalTemplate != null) {
            ...
            if (stateMachineConfig instanceof AbstractStateMachineConfig
                && !((AbstractStateMachineConfig) stateMachineConfig).isSagaBranchRegisterEnable()) {
    ...
    private boolean isUpdateMode(StateInstance stateInstance, ProcessContext context) {
        StateMachineConfig config = (StateMachineConfig) context.getVariable(DomainConstants.VAR_NAME_STATEMACHINE_CONFIG);
        if (config instanceof AbstractStateMachineConfig) {
            AbstractStateMachineConfig stateMachineConfig = ((AbstractStateMachineConfig) config);
