...
public class ConsulConfiguration extends AbstractConfiguration {
	...
	private ConsulConfiguration() {
		...
		token = FILE_CONFIG.getConfig(FILE_CONFIG_KEY_PREFIX + ACL_TOKEN);
	}
	...
	public String getConfig(String dataId, String defaultValue, long timeoutMills) {
		...
		if (StringUtils.isBlank(token)) {
			consulNotifierExecutor.execute(() -> complete(client.getKVValue(dataId), configFuture));
		} else {
			consulNotifierExecutor.execute(() -> complete(client.getKVValue(dataId, token), configFuture));
		}
	...
	public boolean putConfig(String dataId, String content, long timeoutMills) {
		...
		if (StringUtils.isBlank(token)) {
			consulNotifierExecutor.execute(() -> complete(client.setKVValue(dataId, content), configFuture));
		} else {
			PutParams putParams = new PutParams();
			putParams.setCas(CAS);
			consulNotifierExecutor
					.execute(() -> complete(client.setKVValue(dataId, content, token, putParams), configFuture));
		}
		...
		public void onChangeEvent(ConfigurationChangeEvent event) {
			if (null != listener) {
				while (true) {
					...
					if (StringUtils.isBlank(token)) {
						response = client.getKVValue(this.dataId, queryParams);
					} else {
						response = client.getKVValue(this.dataId, token, queryParams);
					}
