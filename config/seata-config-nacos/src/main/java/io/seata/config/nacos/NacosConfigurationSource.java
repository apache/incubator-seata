...
/**
 * The type Nacos configuration source.
 *
 * @author slievrly
 * @author xingfudeshi@gmail.com
 */
public class NacosConfigurationSource implements RemoteConfigurationSource
        , UpdatableConfiguration, ConfigListenerManager {

    private static volatile NacosConfigurationSource instance;

    private static final Logger LOGGER = LoggerFactory.getLogger(NacosConfigurationSource.class);
     ...
     * @return instance
     */
    public static NacosConfigurationSource getInstance() {
        if (instance == null) {
            synchronized (NacosConfigurationSource.class) {
                if (instance == null) {
                    instance = new NacosConfigurationSource();
                }
            }
        }
        return instance;
    ...
    private static Properties getConfigProperties() {
        ...
        if (StringUtils.isNotBlank(userName)) {
            ...
            String accessKey = StringUtils.isNotBlank(System.getProperty(ACCESS_KEY)) ?
                    System.getProperty(ACCESS_KEY) : ConfigurationFactory.getInstance().getString(getNacosAccessKey());
            if (StringUtils.isNotBlank(accessKey)) {
                String secretKey = StringUtils.isNotBlank(System.getProperty(SECRET_KEY)) ?
                        System.getProperty(SECRET_KEY) : ConfigurationFactory.getInstance().getString(getNacosSecretKey());
    ...
    private static String getNacosGroup() {
        return ConfigurationFactory.getInstance().getString(getNacosGroupKey(), DEFAULT_GROUP);
    }

    private static String getNacosDataId() {
        return ConfigurationFactory.getInstance().getString(getNacosDataIdKey(), DEFAULT_DATA_ID);
    }
