...
/**
 * The type Zookeeper config source.
 *
...
public class ZookeeperConfigSource implements RemoteConfigSource,
    ...
    public ZookeeperConfigSource() {
        if (zkClient == null) {
            synchronized (ZookeeperConfigSource.class) {
                if (zkClient == null) {
    ...
    public static class ZKListener implements IZkDataListener {
         ...
         */
        public ZKListener(String path, ConfigurationChangeListener listener, ConfigSource source) {
            this.path = path;
