...
import static io.seata.common.ConfigurationKeys.SEATA_FILE_ROOT_CONFIG;

/**
 * The type Zookeeper configuration source.
 *
 * @author crazier.huang
 */
public class ZookeeperConfigurationSource implements RemoteConfigurationSource, UpdatableConfiguration, ConfigListenerManager {
    private final static Logger LOGGER = LoggerFactory.getLogger(ZookeeperConfigurationSource.class);
    ...
    public ZookeeperConfigurationSource() {
        if (zkClient == null) {
            synchronized (ZookeeperConfigurationSource.class) {
                if (zkClient == null) {
                    ...
                    String username = FILE_CONFIG.getString(FILE_CONFIG_KEY_PREFIX + AUTH_USERNAME);
                    String password = FILE_CONFIG.getString(FILE_CONFIG_KEY_PREFIX + AUTH_PASSWORD);
    ...
    public String getLatestConfig(String dataId, long timeoutMills) {
        ...
        FutureTask<String> future = new FutureTask<>(() -> {
            ...
            return StringUtils.isNullOrEmpty(value1) ? null : value1;
        });
        CONFIG_EXECUTOR.execute(future);
        try {
            return future.get(timeoutMills, TimeUnit.MILLISECONDS);
        } catch (Exception e) {
            String errorMsg = "get remote config '" + dataId + "' error or timeout";
            LOGGER.error(errorMsg + ", exception: {}", e.getMessage());
    ...
    private static String getConfigPath() {
        return FILE_CONFIG.getString(FILE_CONFIG_KEY_PREFIX + CONFIG_PATH_KEY, DEFAULT_CONFIG_PATH);
    }
    ...
    private ZkSerializer getZkSerializer() {
        ...
        String serializer = FILE_CONFIG.getString(FILE_CONFIG_KEY_PREFIX + SERIALIZER_KEY);
