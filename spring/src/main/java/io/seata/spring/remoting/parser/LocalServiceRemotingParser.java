...
/**
 * Local Service Remote Bean Parser
 *
 * @author ruishansun
 */
public class LocalServiceRemotingParser extends AbstractedRemotingParser {
    ...
    public RemotingDesc getServiceDesc(Object bean, String beanName) throws FrameworkException {
        ...
        for (Class<?> interClass : interfaceClasses) {
            if (interClass.isAnnotationPresent(LocalService.class)) {
    ...
    /**
     * Determine whether there is an annotation {@link LocalService}
     *
     * @param bean the bean
     * @return boolean
     */
    private boolean isLocalService(Object bean) {
        Class<?> classType = bean.getClass();
        Set<Class<?>> interfaceClasses = ReflectionUtil.getInterfaces(classType);
        for (Class<?> interClass : interfaceClasses) {
            if (interClass.isAnnotationPresent(LocalService.class)) {
