...
public class RBACRequestAuthHeaderProvider implements AuthHeaderProvider {
    ...
    public RBACRequestAuthHeaderProvider() {
        ...
        if (enabled()) {
            ...
            executorService = new ThreadPoolExecutor(1, 1, 0L, TimeUnit.MILLISECONDS,
                new LinkedBlockingQueue<Runnable>(), t -> new Thread(t, "rbac-executor"));
            ...
            cache = CacheBuilder.newBuilder().maximumSize(1).refreshAfterWrite(refreshTime(), TimeUnit.MILLISECONDS)
                .build(new CacheLoader<String, String>() {
                    ...
                    public ListenableFuture<String> reload(String key, String oldValue) {
                        return Futures.submitAsync(Callables.asAsyncCallable(() -> createHeaders(), service),
                            executorService);
                    }
                });
        }
    ...
    private boolean enabled() {
        return !StringUtils.isEmpty(properties.getConfig(SeataServicecombKeys.KEY_RBAC_NAME))
            && !StringUtils.isEmpty(properties.getConfig(SeataServicecombKeys.KEY_RBAC_PASSWORD));
    }
