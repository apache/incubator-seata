...
polarisAddr=$host:$port
contentType="content-type:application/json;charset=UTF-8"

echo "set polarisAddr=$polarisAddr"
...
addConfig() {
  dataId=`urlencode $1`
  content=`urlencode $2`
  curl -X POST -H "${contentType}" "http://$polarisAddr/nacos/v1/cs/configs?dataId=$dataId&group=$group&content=$content&tenant=$tenant&username=$username&password=$password" >"${tempLog}" 2>/dev/null
