...
public class McpAsyncServer {
	...
	public Mono<Void> addTool(McpServerFeatures.AsyncToolSpecification toolSpecification) {
		...
		return Mono.defer(() -> {
			...
			if (this.tools.stream().anyMatch(th -> th.tool().getName().equals(wrappedToolSpecification.tool().getName()))) {
				return Mono.error(
						new McpError("Tool with name '" + wrappedToolSpecification.tool().getName() + "' already exists"));
			}
	...
	private static class StructuredOutputCallToolHandler
		...
		public Mono<CallToolResult> apply(McpAsyncServerExchange exchange, CallToolRequest request) {
			...
			return this.delegateCallToolResult.apply(exchange, request).map(result -> {
				...
				JsonSchemaValidator.ValidationResponse validation = this.jsonSchemaValidator.validate(outputSchema, result.getStructuredContent());
				...
				if (Utils.isEmpty(result.getContent())) {
					...
					return new CallToolResult(Collections.singletonList(new TextContent(validation.getJsonStructuredOutput())),
							result.getError(), result.getStructuredContent());
				}
	...
	public Mono<Void> removeTool(String toolName) {
		...
		return Mono.defer(() -> {
			boolean removed = this.tools
				.removeIf(toolSpecification -> toolSpecification.tool().getName().equals(toolName));
			if (removed) {
	...
	private McpRequestHandler<ListResourceTemplatesResult> resourceTemplateListRequestHandler() {
		return (exchange, params) -> Mono
			.just(new ListResourceTemplatesResult(null, this.getResourceTemplates()));
	...
	private List<ResourceTemplate> getResourceTemplates() {
		...
		List<ResourceTemplate> resourceTemplates = this.resources.keySet()
			...
			.map(uri -> {
				...
				ResourceTemplate template = new ResourceTemplate(resource.getUri(), resource.getName(),
	...
	public Mono<Void> addPrompt(McpServerFeatures.AsyncPromptSpecification promptSpecification) {
		...
		return Mono.defer(() -> {
			...
			if (specification != null) {
				return Mono.error(
						new McpError("Prompt with name '" + promptSpecification.prompt().getName() + "' already exists"));
			}
