...
Vite 开发服务器默认端口为 `5173`，可通过 `--port` 参数修改，通过 `--host 0.0.0.0` 开放网络访问。
...
如需在本地开发时启用 API 代理，可在 `vite.config.ts` 中添加以下配置：
...
> **注意**：开发阶段的 API 代理为可选配置，已从 `vite.config.ts` 中移除。生产部署时由后端网关或 Nginx 处理反向代理。
