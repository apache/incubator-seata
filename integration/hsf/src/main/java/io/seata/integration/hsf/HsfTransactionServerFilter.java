...
/**
 * hsf Transaction propagation filter server side.
 *
 * @author slievrly
 */
public class HsfTransactionServerFilter implements ServerFilter {

    private static final Logger LOGGER = LoggerFactory.getLogger(HsfTransactionServerFilter.class);
    ...
    public ListenableFuture<RPCResult> invoke(InvocationHandler invocationHandler, Invocation invocation)
        ...
        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug("xid in Server.RpcContext[{}]", rpcXid);
        }
        boolean bind = false;
        if (rpcXid != null) {
