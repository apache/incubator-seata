...
public class OracleUndoLogManagerTest {
     ...
     * Oracle 默认使用 undo_log 表和 undo_log_SEQ 序列
     */
    @Test
    public void testDefaultTableNameSequenceGeneration() throws Exception {
        ...
        // 验证默认情况下使用 UNDO_LOG_SEQ（大写，遵循Oracle社区约定）
        ...
        // 验证使用 Oracle 特有的时间函数
        Assertions.assertTrue(insertSql.contains("sysdate"), "Oracle 应该使用 sysdate 时间函数，实际SQL: " + insertSql);
    }
    ...
    public void testCustomTableNameSequenceGeneration() throws Exception {
        ...
        try (MockedStatic<ConfigurationFactory> configurationFactoryMock =
            ...
            // 验证序列名生成逻辑 - Oracle约定使用大写序列名
            String customTableName = "my_undo_log";
            String expectedSequenceName = customTableName.toUpperCase() + "_SEQ";  // MY_UNDO_LOG_SEQ，遵循Oracle约定
            ...
            // 验证序列名生成逻辑是否正确 - Oracle约定使用大写
            Assertions.assertTrue(
                    expectedSqlPart.contains("MY_UNDO_LOG_SEQ"), "自定义表名 my_undo_log 应该生成序列名 MY_UNDO_LOG_SEQ（遵循Oracle大写约定）");
            
            // 验证确实转换为了大写（Oracle社区约定）
            Assertions.assertFalse(
                    expectedSqlPart.contains("my_undo_log_SEQ"), "Oracle应该将序列名转换为大写，而不是保持小写");
        }
    }
    ...
    public void testSequenceNameGenerationRules() {
        ...
        for (String[] testCase : testCases) {
            ...
            // 验证序列名生成规则 - Oracle约定转大写
            String actualSequence = tableName.toUpperCase() + "_SEQ";
            Assertions.assertEquals(
                    expectedSequence,
                    actualSequence,
                    String.format("表名 '%s' 应该生成序列名 '%s'（遵循Oracle大写约定）", tableName, expectedSequence));
        }
    }
    ...
    public void testBackwardCompatibility() throws Exception {
        ...
        Assertions.assertTrue(insertSql.contains("sysdate"), "应该保持 Oracle 时间函数 sysdate");
    ...
    public void testFixComparison() {
        ...
        // 修复后（基于表名动态生成，遵循Oracle大写约定）
        String defaultTableName = "undo_log";
        String newSequenceName = defaultTableName.toUpperCase() + "_SEQ";
        System.out.println("修复后: " + newSequenceName + ".nextval - 基于表名动态生成，遵循Oracle大写约定");
        ...
        System.out.println("自定义表名: " + customSequenceName + ".nextval - 支持自定义且遵循Oracle大写约定");
        
        // 验证转换为大写（遵循Oracle社区约定）
        Assertions.assertEquals("MY_UNDO_LOG_SEQ", customSequenceName, "自定义表名应该转换为大写序列名");
        Assertions.assertNotEquals("my_undo_log_SEQ", customSequenceName, "Oracle应该将序列名转换为大写");
    }
