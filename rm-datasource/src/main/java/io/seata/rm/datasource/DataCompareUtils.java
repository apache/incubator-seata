...
public class DataCompareUtils {
        ...
        if (StringUtils.equalsIgnoreCase(f0.getName(), f1.getName())
          ...
          if (f0.getValue() == null) {
            ...
            if (f1.getValue() == null) {
              ...
              String currentSerializer= UndoLogManagerFactory.getDefaultInstance().getCurrentSerializer();
  ...
  private static Result<Boolean> compareRows(
    ...
    for (String rowKey : oldRowsMap.keySet()) {
      ...
      if (newRow == null) {
        return Result.buildWithParams(
            false, "compare row failed, rowKey {}, reason [newRow is null]", rowKey);
      }
  ...
  private static Map<String, Map<String, Field>> rowListToMap(
      List<Row> rowList, String primaryKey) {
