...
public class ConnectionProxy extends AbstractConnectionProxy {
  ...
  private static int REPORT_RETRY_COUNT =
      ConfigurationFactory.getInstance()
          .getInt(ConfigurationKeys.CLIENT_REPORT_RETRY_COUNT, DEFAULT_REPORT_RETRY_COUNT);
    ...
    try {
      boolean lockable =
          DefaultResourceManager.get()
              .lockQuery(
                  BranchType.AT, getDataSourceProxy().getResourceId(), context.getXid(), lockKeys);
      if (!lockable) {
  ...
  private void processGlobalTransactionCommit() throws SQLException {
    ...
    try {
      if (context.hasUndoLog()) {
        UndoLogManagerFactory.getInstance(this.getDbType()).flushUndoLogs(this);
      }
      targetConnection.commit();
    } catch (Throwable ex) {
